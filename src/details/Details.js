...
function handleHome() {
  ...
  return (
    <div>
      ...
      >
        <Box sx={style} className={"card "+typeData[0]+" "+typeData[1]} fontFamily={[ 'monospace']}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 1 }} fontFamily={['monospace']} display={"flex"} gap={"25px"}>
          {pokemon.types.map((type)=>{return <div key={type} className={"type-"+ type}>{dataType[type][lang]}</div>})} 
          </Typography> 
          <Typography id="modal-modal-title" variant="h6" component="h2">
           Poids : {pokemonWeight} kg Taille : {pokemonHeight} m <br/> Attaques:
          </Typography>
          <Typography id="modal-modal-description" sx={{ mt: 2, display:"flex", flexWrap:"wrap", gap:"15px"}} fontFamily={['monospace']}>
